...
class AdvancedScene(Scene):
    ...
    def construct(self) -> None:
        ...
        axes.move_to([2.92, 0.37, 0.0])
        self.play(Create(axes), run_time=1.0)
        sine_curve = axes.plot(lambda x: np.sin(x * PI), x_range=[-1, 3], color=BLUE)
        self.play(Create(sine_curve), run_time=1.5)
        equation = MathTex('f(x) = \\sin(\\pi x)', font_size=28, color=WHITE, fill_opacity=1.0)
        equation.scale(2.6)
        equation.rotate(0.0)
        equation.move_to([-3.13, 2.51, 0.0])
        equation.next_to(axes, UP, buff=0.3)
        self.play(DrawBorderThenFill(equation), run_time=1.0)
        circle = Circle(radius=1.2, color=RED, fill_opacity=0.4, stroke_opacity=1.0, stroke_width=0.0)
        circle.rotate(2.8)
        circle.scale(10.0)
        circle.move_to([5.45, -2.12, 0.0])
        self.play(GrowFromCenter(circle), run_time=0.8)
        square = Square(side_length=1.7, color=BLUE, fill_opacity=0.1, stroke_opacity=0.8, stroke_width=8.4)
        square.rotate(-2.1)
        square.scale(0.1)
        square.move_to([5.39, 0.34, 0.0])
        self.play(SpinInFromNothing(square), run_time=1.0)
        triangle = Triangle(color=PURPLE, fill_opacity=1.0, stroke_opacity=0.0, stroke_width=20.0)
        triangle.rotate(6.2)
        triangle.scale(2.3)
        triangle.move_to([1.92, 1.22, 0.0])
        ...
        dot = Dot(axes.c2p(0.5, np.sin(0.5 * PI)), radius=0.1, color=YELLOW, stroke_opacity=1.0, stroke_width=8.3, fill_opacity=1.0)
        dot.rotate(-5.6)
        dot.scale(0.1)
        dot.move_to([-3.06, 0.2, 0.0])
        self.play(Write(dot, scale=3), run_time=0.5)
